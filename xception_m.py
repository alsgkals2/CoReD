...
class Xception(nn.Module):
    ...
    def features(self, input):
        ...
        feat5 = self.block5(feat4)
        feat6 = self.block6(feat5)
        feat7 = self.block7(feat6)
        feat8 = self.block8(feat7)
        feat9 = self.block9(feat8)
        feat10 = self.block10(feat9)
        feat11 = self.block11(feat10)
        feat12 = self.block12(feat11) #(1024, 299, 299)

        x = self.conv3(feat12) #(1536, 299, 299)
        x = self.bn3(x)
        x = self.relu(x)

        x = self.conv4(x) #(2048, 299, 299)
        x = self.bn4(x)
        feats = (feat1,feat2,feat3,feat4,feat5,feat6,feat7,feat8,feat9,feat10,feat11,feat12)
        #temp
        cnt=0
        for f in feats:
            print(f'feat{cnt} shape ---> ', f.shape)
            cnt+=1
        return x, feats


    def logits(self, features):
        x,feats = self.relu(features)
